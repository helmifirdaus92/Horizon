...
import { LayoutDataRequestContext } from 'app/editor/editing-metadata/editing-metadata.service';
import { EditingData } from 'app/editor/editing-metadata/layout-service-models';
import { LayoutContainerEditKind } from 'app/editor/right-hand-side/layout-container-details/layout-container-rendering-service';
import { MediaSelectionResult } from 'app/editor/right-hand-side/rich-text-editor/media-picker.service';
...
import { ItemPermissions } from '../graphql/item.interface';
...
export interface FlowDefinition {
  name: string;
  friendlyId: string;
  status: 'PRODUCTION' | 'PAUSED' | 'DRAFT' | 'PUBLISHING' | 'COMPLETED';
}
...
export interface EditingMetadataHorizonRpcServices {
  getEditingMetadata(editingDataContext: LayoutDataRequestContext): EditingData;
}
