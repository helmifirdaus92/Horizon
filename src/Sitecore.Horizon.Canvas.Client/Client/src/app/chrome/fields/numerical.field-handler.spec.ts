...
import { NumericalFieldHandler, RhsChannel, RhsChannelDef } from './numerical.field-handler';

/* eslint-disable @typescript-eslint/no-non-null-assertion */

describe(NumericalFieldHandler.name, () => {
  let sut: NumericalFieldHandler;
  ...
  beforeEach(() => {
    ...
    sut = new NumericalFieldHandler(sutElement, rawValueHolder, rhsMessaging, abortController);
    sut.init();
  });
  ...
  describe('onSelect() callback', () => {
    ...
    it('should dispatch RHS value:change message on change', async () => {
      const newValue: FieldRawValue = { rawValue: '10.0' };
  ...
  describe('readValue', () => {
    it('should include rawValue', () => {
      sut.setValue({ rawValue: '1.5' });

      expect(sut.getValue()).toEqual({ rawValue: '1.5' });
    });
  });

  describe('writeValue', () => {
    it('should update rawValue', () => {
      sut.setValue({ rawValue: '10.0' });

      expect(sut.getValue().rawValue).toBe('10.0');
      expect(sutElement.innerText).toBe('10.0');
    });

    it('should update rawValue to rawValueHolder', () => {
      sut.setValue({ rawValue: '10.0' });

      expect(rawValueHolder.value).toBe('10.0');
    });
    ...
    it('should call onChange', () => {
      ...
      expect(receivedValue).toEqual({ value: { rawValue: '10.0' }, debounce: false });
      expect(sutElement.innerText).toBe('10.0');
    });
  });
  ...
  describe('messaging', () => {
    ...
    it('should set value', async () => {
      await messaging.registeredRpcServicesImpl!.setValue('10.0');
