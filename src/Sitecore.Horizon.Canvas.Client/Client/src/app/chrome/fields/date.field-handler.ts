...
export class DateFieldHandler implements FieldHandler {
  ...
  async setValue(value: FieldRawValue): Promise<void> {
    if (!value.rawValue) {
      this.element.innerHTML = '';
    }
    this.setWatermarkIfEmpty();
    ...
    // JSS DateField render the date/date-time format based on user configuration
    // Empty value can be rendered by ourselves
    const requireSSR = !!value.rawValue;
