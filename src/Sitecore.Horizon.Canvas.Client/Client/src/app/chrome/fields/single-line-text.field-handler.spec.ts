...
describe(SingleLineTextFieldHandler.name, () => {
  let sut: SingleLineTextFieldHandler;
  ...
  beforeEach(async () => {
    ...
    sut = new SingleLineTextFieldHandler(sutElement, rawValueHolder, abortController);
    sut.init();
  });
  ...
  it('should return decoded html for readValue', () => {
    sutElement.innerHTML = '';
    sutElement.contentEditable = 'true';
    sutElement.focus();

    document.execCommand('insertText', true, 'text with space ');

    const result = sut.getValue();
    expect(sutElement.innerHTML).toContain('&nbsp;');
    // it should contain %C2%A0 as no breaking space
    expect(encodeURI(result.rawValue)).toBe('text%20with%20space%C2%A0');
  });
  ...
  it('getValue() should empty string if scdefaulttext', () => {
    const scDefaultText = sut.fieldWatermarkText;

    sutElement.innerText = scDefaultText;
  ...
  describe('when no text in the field', () => {
    ...
    it('should remove placeholder value from field when focused via TAB navigation', async () => {
      // arrange
    ...
    it('should display watermark value if value is empty when loosing focus', async () => {
      // arrange
      const scDefaultText = sut.fieldWatermarkText;
      ...
      // assert
      expect(sutElement.innerText).toBe(scDefaultText);
    });
  });
