...
This repository contains BDD tests in two primary formats: traditional Gherkin `.feature` files and C# test classes and methods that are structured following BDD principles (feature-per-file, scenario-per-method).

## Gherkin Feature Files

### `tests/Sitecore.Horizon.Integration.Editor.Tests.UI/Features/FieldsEditing/RichTextField.feature`
```gherkin
﻿@Smoke
@RichTextField
...
## C# BDD-style Tests (from `tests/SItecore.Pages.E2EFramework.UI.Tests/Features/`)

The following tree represents the features identified from C# test files located in the `tests/SItecore.Pages.E2EFramework.UI.Tests/Features/` directory and its subdirectories. Each item typically corresponds to a C# file name, with the hierarchy representing the directory structure.
